...
<div class="card shadow mb-4">
  ...
  <div class="card-body">
    ...
    <form action="{{ route('post.proses-tambah.stok') }}" method="post">
      @csrf
      ...
      <div class="form-group row">
        ...
        <div class="col-sm-10">
          <input type="text" class="form-control @error('jumlah_stok') is-invalid @enderror" name="jumlah_stok" value="{{ old('jumlah_stok', '') }}">
        ...
        <div class="col-sm-10">
          <select class="admin-id form-control custom-select" name="admin_ke">
            ...
            @foreach($data_admin as $admin)
              <option value="{{ $admin->id }}" {{ old('admin_id') == $admin->id ? 'selected' : '' }}>{{ $admin->nama }}</option>
            @endforeach
          </select>
